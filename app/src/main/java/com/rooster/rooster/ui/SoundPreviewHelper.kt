...
class SoundPreviewHelper(private val context: Context) {
    ...
    fun previewSound(ringtoneUri: String, durationMs: Long = 3000) {
        ...
        Logger.d("SoundPreviewHelper", "Previewing sound with URI: $ringtoneUri")
        
        val uri = when {
            ringtoneUri.isEmpty() || ringtoneUri.equals("default", ignoreCase = true) || ringtoneUri == "Default" -> {
                Logger.d("SoundPreviewHelper", "Using default ringtone")
                null // Use default system ringtone
            }
            else -> {
                try {
                    val parsedUri = Uri.parse(ringtoneUri)
                    Logger.d("SoundPreviewHelper", "Parsed URI: $parsedUri")
                    parsedUri
                } catch (e: Exception) {
                    Logger.e("SoundPreviewHelper", "Error parsing URI: $ringtoneUri", e)
                    null
                }
            }
        }
        ...
        if (uri != null && isSystemRingtoneUri(uri)) {
            try {
                ...
                if (rt != null) {
                    Logger.d("SoundPreviewHelper", "Using RingtoneManager for system ringtone")
                    ringtone = rt
                    ringtone?.play()
                    isPlayingVar = true
                    ...
                    stopHandler?.postDelayed({
                        Logger.d("SoundPreviewHelper", "Auto-stopping preview after $durationMs ms")
                        stopPreview()
                    }, durationMs)
                    return
                }
            } catch (e: Exception) {
                Logger.w("SoundPreviewHelper", "Could not play with RingtoneManager, trying MediaPlayer", e)
            }
        }
        ...
        try {
            ...
            val player = MediaPlayer().apply {
                ...
                try {
                    ...
                    Logger.d("SoundPreviewHelper", "DataSource set successfully")
                } catch (e: Exception) {
                    Logger.e("SoundPreviewHelper", "Error setting data source with URI: $actualUri", e)
                    throw e
                }
                ...
                setOnPreparedListener { mp ->
                    try {
                        Logger.d("SoundPreviewHelper", "MediaPlayer prepared, starting playback")
                        if (mp == mediaPlayer) { // Only start if this is still the current player
                            mp.start()
                            isPlayingVar = true
                            Logger.d("SoundPreviewHelper", "Playback started")
                            ...
                            Logger.w("SoundPreviewHelper", "MediaPlayer reference changed, not starting")
                        }
                    } catch (e: Exception) {
                        Logger.e("SoundPreviewHelper", "Error starting preview", e)
                        stopPreview()
                    }
                }
                
                setOnCompletionListener {
                    Logger.d("SoundPreviewHelper", "Playback completed")
                    stopPreview()
                }
                
                setOnErrorListener { _, what, extra ->
                    Logger.e("SoundPreviewHelper", "MediaPlayer error - What: $what, Extra: $extra")
                    stopPreview()
                    true
                }
                
                Logger.d("SoundPreviewHelper", "Starting async preparation")
                prepareAsync()
            }
            
            mediaPlayer = player
        } catch (e: Exception) {
            Logger.e("SoundPreviewHelper", "Error previewing sound", e)
            e.printStackTrace()
            stopPreview()
        }
    }
    ...
    fun stopPreview() {
        ...
        ringtone?.let { rt ->
            try {
                ...
                Logger.w("SoundPreviewHelper", "Error stopping Ringtone", e)
            }
        }
        ringtone = null
            ...
            try {
                ...
                if (player.isPlaying) {
                    try {
                        ...
                        Logger.w("SoundPreviewHelper", "Error stopping MediaPlayer", e)
                    }
                }
                ...
                try {
                    ...
                    Logger.w("SoundPreviewHelper", "Error resetting MediaPlayer", e)
                }
                ...
                try {
                    ...
                    Logger.w("SoundPreviewHelper", "Error releasing MediaPlayer", e)
                }
            } catch (e: Exception) {
                Logger.e("SoundPreviewHelper", "Error in MediaPlayer cleanup", e)
            }
        }
    }
